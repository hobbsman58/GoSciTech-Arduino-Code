...
// Bump behavior
const byte STRAIGHT_SPEED = 100;
const byte FORWARD_SPEED = 25;   // Define normal speeds
...
void loop() {
  ...
  timer++;
  if(timer>=25, brightLeft<leftIR, darkRight>rightIR) {
    motorLeft->setSpeed(STRAIGHT_SPEED);
    motorRight->setSpeed(STRAIGHT_SPEED);
     motorLeft->run(FORWARD);
     motorRight->run(FORWARD);
  } 
  if(timer>=25) {
